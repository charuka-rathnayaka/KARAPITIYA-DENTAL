...
class DataLayer{
	...
	function arrivalApproved(){
        ...
        if (mysqli_num_rows($re) > 0) {
            while ($rows = mysqli_fetch_assoc($re)) {
                echo "<form class='tbl' method='POST' id='appointmentdata' ><input type='hidden' name='table' value='waitinglist'><tr><td>" . $rows["username"] . "<input type='hidden' name='username' value='" . $rows["username"] . "'></td><td>" . $rows["patient_name"] . "<input type='hidden' name='patient_name' value='" . $rows["patient_name"] . "'></td><td>" . $rows["appointment_ID"] . "<input type='hidden' name='appointment_ID' value='" . $rows["appointment_ID"] . "'></td><td>" . $rows["date"] . "<input type='hidden' name='date' value='" . $rows["date"] . "'></td><td >" . $rows["Appointmentnumber"] . "<input type='hidden' name='appointmentnumber' value='" . $rows["Appointmentnumber"] . "'></td><td >" . $rows["category"] . "<input type='hidden' name='category' value='" . $rows["category"] . "'></td></tr></form>";
            }
        }
	}
}
